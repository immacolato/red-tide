...
export class SpawnSystem {
  ...
  calculateClientPatience() {
    ...
    const basePatience =
      basePatienceMin + Math.random() * (basePatienceMax - basePatienceMin);
    
    // Riduzione per affollamento
    const patience =
      basePatience * (1 - crowdFactor * Config.CLIENT.CROWD_PATIENCE_PENALTY);
  ...
  getStats() {
    return {
      ...
      nextSpawnIn: (this.gameState.spawnInterval / this.getSpawnMultiplier() - this.spawnTimer).toFixed(1),
    };
  }

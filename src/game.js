...
function update(dt) {
  ...
  if (
    Math.floor(state.time) % 30 === 0 &&
    Math.floor(state.time) > 0 &&
    Math.floor(state.time) !== state.lastPriceFeedback
  ) {
    ...
    if (priceAttractiveness > 0.8) {
      log(
        '📊 I tuoi prezzi attraggono molti clienti! Considera se aumentare leggermente per più profitto'
      );
    } else if (priceAttractiveness < 0.4) {
      log(
        '📊 I prezzi sono alti, pochi clienti si avvicinano. Considera di ridurre per aumentare le vendite'
      );
  ...
  for (let i = state.clients.length - 1; i >= 0; i--) {
    ...
    if (c.timeAlive > 150) {
      // Timeout assoluto di 2.5 minuti
      state.clients.splice(i, 1);
...
function render() {
  ...
  for (const c of state.clients) {
    ...
    ctx.fillRect(c.x - 2, c.y + c.r + 2, 4, 2);
  }
  ...
  ctx.fillStyle = '#fff';
  ctx.font = '14px sans-serif';
  ctx.fillText('€' + state.money.toFixed(2), 16, 30);
  ctx.font = '12px sans-serif';
  ctx.fillStyle = '#ccc';
  ctx.fillText('Clienti: ' + state.clients.length, 120, 30);
  ...
  container.querySelectorAll('button').forEach(b => {
    b.onclick = e => {
      ...
      if (action === 'dec') {
        ...
        if (markup < 0.3) {
          log(
            `💰 ${prod.name}: prezzo ridotto a €${prod.price.toFixed(2)} - Molto attraente per i clienti!`
          );
        } else if (markup < 0.8) {
          log(
            `💰 ${prod.name}: prezzo ridotto a €${prod.price.toFixed(2)} - Buon equilibrio prezzo/profitto`
          );
        } else {
          log(
            `💰 ${prod.name}: prezzo ridotto a €${prod.price.toFixed(2)} - Ancora costoso ma più accessibile`
          );
        }
        renderItemsPanel();
        updateHUD();
      ...
      if (action === 'inc') {
        ...
        if (markup > 2.0) {
          log(
            `💰 ${prod.name}: prezzo aumentato a €${prod.price.toFixed(2)} - Clienti potrebbero evitare il prodotto`
          );
        } else if (markup > 1.2) {
          log(
            `💰 ${prod.name}: prezzo aumentato a €${prod.price.toFixed(2)} - Profitto alto ma meno clienti`
          );
        } else {
          log(
            `💰 ${prod.name}: prezzo aumentato a €${prod.price.toFixed(2)} - Equilibrio ragionevole`
          );
        }
        renderItemsPanel();
        updateHUD();
...
function updateHUD() {
  ...
  document.getElementById('price-attractiveness-value').textContent =
    priceAttractiveness.toFixed(0);
...
function setupEventListeners() {
  ...
  if (marketingBtn) {
    marketingBtn.onclick = () => {
      ...
      if (state.money < cost) {
        log('Soldi insufficienti per marketing (€' + cost + ')');
        return;
      }
      state.money -= cost;
  ...
  if (restockBtn) {
    restockBtn.onclick = () => {
      ...
      if (state.money < cost) {
        log('Non hai abbastanza soldi per rifornire tutto (serve €' + cost.toFixed(2) + ')');
        return;
      }
      state.money -= cost;
  ...
  document.getElementById('expand').onclick = () => {
    const cost = 200;
    if (state.money < cost) {
      log('Non hai abbastanza soldi per espandere');
      return;
    }
    state.money -= cost;
    state.clientCap += 10;
